...
static void* const ncclKerns[NCCL_NUM_FUNCTIONS*ncclNumOps*ncclNumTypes*NCCL_NUM_ALGORITHMS*NCCL_NUM_PROTOCOLS] = {
  ...
  NCCL_FUNCS2A(ncclAllReduceComputation) 
};
...
ncclResult_t ncclBarrierEnqueueWait(ncclComm_t comm) {
  ...
  if (comm->launchMode == ncclComm::PARALLEL) {
    dim3 gridDim = params->gridDim;
    // gridDim.x *= 1;
    CUDACHECK(cudaLaunchCooperativeKernel(params->func, gridDim, params->blockDim, params->args, params->sharedMem, params->stream));
  }
...
static ncclResult_t getPatternInfo(struct ncclInfo* info) {
  switch (info->coll) {
    ...
    case ncclCollAllReduceComputation:
      info->pattern = info->algorithm == NCCL_ALGO_COLLNET ? ncclPatternCollTreeUp : info->algorithm == NCCL_ALGO_TREE ? ncclPatternTreeUpDown : ncclPatternRingTwice; break;
    default:
...
static ncclResult_t getLoopInfo(struct ncclInfo* info) {
  switch (info->pattern) {
    ...
    case ncclPatternRingTwice:
      info->nstepsPerLoop = ((info->coll == ncclCollAllReduceComputation) ? 2 : 1)*2*(info->comm->nRanks-1); info->nchunksPerLoop = info->comm->nRanks; break;
    default:
...
static ncclResult_t computeColl(struct ncclInfo* info /* input */, struct ncclColl* coll, struct ncclProxyArgs* proxyArgs /* output */) {
  ...
  coll->args.lr = info->fusedComputation.lr;
coll->args.beta1 = info->fusedComputation.beta1;
coll->args.beta2 = info->fusedComputation.beta2;
coll->args.g = info->fusedComputation.g;
coll->args.w = info->fusedComputation.w;
coll->args.halfw = info->fusedComputation.halfw;
coll->args.m = info->fusedComputation.m;
coll->args.v = info->fusedComputation.v;
coll->args.r = info->fusedComputation.r;
...
static ncclResult_t saveKernel(struct ncclInfo* info) {
  if (info->comm->nRanks == 1) {
    //if (true)//(info->sendbuff != info->recvbuff)
...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  INFO(NCCL_COLL,"%s: opCount %lx count %zi datatype %d op %d root %d comm %p [nranks=%d] stream %p",
       info->opName, info->comm->opCount, info->count,
