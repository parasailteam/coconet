...
static const char* ncclFuncStr[] = { "Broadcast", "Reduce", "AllGather", "ReduceScatter", "AllReduce", "AllReduceComputation" };
...
static const double ll128MaxBw[NCCL_NUM_FUNCTIONS] = { 113.0, 72.0, 110.0, 91.0, 100.0, 100.0 };
...
ncclResult_t ncclTopoSetThresholds(struct ncclComm* comm, int minCompCap, int maxCompCap, struct ncclTopoGraph* treeGraph, struct ncclTopoGraph* ringGraph, struct ncclTopoGraph* collNetGraph) {
  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    int nsteps = (coll == ncclCollAllReduce || coll == ncclCollAllReduceComputation) ? 2*(comm->nRanks-1) :
      coll == ncclCollReduceScatter || coll == ncclCollAllGather ? comm->nRanks-1 :
      comm->nRanks;

    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      if ((coll != ncclCollAllReduce && coll != ncclCollAllReduceComputation) && a != NCCL_ALGO_RING) continue;
