...
__device__ inline int smidToBid1(int smid, int nChannels) {
  //Assigns 
  int bid = smid/1;
  if (smid % 1 != 0 || bid >= nChannels) { 
    return -1;
  }
...
__device__ inline int smidToBidDefault(int smid, int nChannels) {
  //Assigns block to even SMs
  int bid = smid/2;
  if (smid % 2 != 0 || bid >= nChannels) { 
    return -1;
  }
...
__device__ inline int smidToBidOdd(int smid, int nChannels) {
  //Assigns block to even SMs
  int bid = smid/2;
  if (smid % 2 != 1 || bid >= nChannels) { 
    return -1;
  }
...
__device__ inline int smidToBid4(int smid, int nChannels) {
  //Assigns block to every Nth SM
  const int N = 4;
  int bid = smid/N;
  if (smid % N != 0 || bid >= nChannels) { 
    return -1;
  }
...
__device__ inline int smidToBid2(int smid, int nChannels) {
  //Divides All SMs into groups of size N.
  //Assign blocks to SMs in even groups only.
  const int groupSize = 2;
  int smGroup = smid / groupSize;
  //int smInGroup = smid % groupSize;
  if (groupSize != 1 && smGroup % groupSize >= groupSize/2)
    return -1;
  int bid = smid - groupSize * (smGroup / groupSize);
  if (bid >= nChannels) { 
    return -1;
  }
...
__device__ inline int smidToBid3(int smid, int nChannels) {
  //Assign first half of blocks to consecutive SMs.
  //Assign next half of blocks to every other SM.
  if (smid < nChannels/2) {
    return smid;
  }
  ...
  int bid = nChannels/2 + (smid - nChannels/2)/2;
...
__global__ void NCCL_KERN_NAME(coll, op, dtype)(struct ncclColl firstColl) { \
  int tid = threadIdx.x; \
  int bid = blockIdx.x; \
  if (gridDim.x > firstColl.args.nChannels) {\
    bid = smidToBid1(getSMID(), firstColl.args.nChannels);\
    if (bid == -1) return;\
  }\
