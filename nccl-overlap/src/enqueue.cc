...
ncclResult_t ncclBarrierEnqueueWait(ncclComm_t comm) {
  ...
  if (comm->launchMode == ncclComm::PARALLEL) {
    ...
    // if (comm->rank == 0) printf("blockDim.x %d gridDim.x %d\n", params->blockDim.x,params->gridDim.x);
    // if (comm->rank == 0) printf("gridDim.x %d\n", params->gridDim.x);
    // params->gridDim.x = 80;
    // printf("launching kernel with gridDim.x %d\n", params->gridDim.x);
    // CUDACHECK(cudaLaunchKernel(params->func, params->gridDim, params->blockDim, params->args, params->sharedMem, params->stream));
  }
...
static ncclResult_t computeColl(struct ncclInfo* info /* input */, struct ncclColl* coll, struct ncclProxyArgs* proxyArgs /* output */) {
  ...
  coll->args.realChunkCols = info->matMulConfig.realChunkCols;
  coll->args.outerIteration = info->matMulConfig.outerIteration;
  coll->args.combinedChunks = info->matMulConfig.combinedChunks;
